...
function createConfigTab(tab, fig, appData)
    ...
    uidropdown(rfPanel, 'Position', [300 130 150 22], ...
        ...
        'Value', '0: BPSK 1/2 (6.5 Mbps)', 'Tag', 'mcsSelect');
    ...
    uieditfield(timingPanel, 'numeric', 'Position', [160 140 80 22], ...
        'Value', 0.10, 'Tag', 'txTime', 'Limits', [0.0 2.0]);
    ...
    uieditfield(timingPanel, 'numeric', 'Position', [350 140 60 22], ...
        'Value', 0.30, 'Tag', 'syncInterval', 'Limits', [0 2.0]);
end
...
function createRxTab(rxTab, fig, appData)
    ...
    uieditfield(settingsPanel, 'numeric', 'Position', [140 80 100 22], ...
        'Value', 80000, 'Tag', 'rxSamplesPerFrame', 'Limits', [4000 200000]);
    ...
    uieditfield(settingsPanel, 'numeric', 'Position', [360 80 50 22], ...
        'Value', 40, 'Tag', 'rxGain', 'Limits', [0 89.75]);
...
function detectSDRs(fig)
    ...
    [availableRadios, statusText] = detectUsrpRadios();
...
function [availableRadios, statusText] = detectUsrpRadios()
    availableRadios = {};
    ...
    if isempty(usrpInfo)
        availableRadios = {'type=b200'};
        statusText = 'Detected SDRs: No USRP enumerated, using fallback selector type=b200';
        return;
    end

    if isstruct(usrpInfo)
        usrpInfo = num2cell(usrpInfo);
    end

    for i = 1:numel(usrpInfo)
        device = usrpInfo{i};
        platform = getStructField(device, 'Platform', 'USRP');
        serialNum = getStructField(device, 'SerialNum', '');
        status = getStructField(device, 'Status', '');

        if contains(upper(platform), 'B200') || contains(upper(platform), 'B210') || contains(upper(platform), 'NI2900')
            if ~isempty(serialNum)
                availableRadios{end+1} = sprintf('serial=%s', serialNum);
            else
                availableRadios{end+1} = 'type=b200';
            end
        elseif contains(upper(status), 'SUCCESS') && ~isempty(serialNum)
            availableRadios{end+1} = sprintf('serial=%s', serialNum);
        end
    end

    availableRadios = unique(availableRadios, 'stable');

    if isempty(availableRadios)
        availableRadios = {'type=b200'};
        statusText = 'Detected SDRs: No B200/NI2900 found, using fallback selector type=b200';
    else
        statusText = sprintf('Detected SDRs: %s', strjoin(availableRadios, ', '));
    end
end

function value = getStructField(structValue, fieldName, defaultValue)
...
function config = getConfiguration(fig)
    ...
    sampleRateField = findobj(fig, 'Tag', 'sampleRate');
    requestedSampleRate = sampleRateField.Value * 1e6;
    config.sampleRate = alignSampleRateWithChannelBW(config.channelBW, requestedSampleRate);
    sampleRateField.Value = config.sampleRate / 1e6;
    ...
    try
        ...
        if config.syncPackets > 0
            ...
            for k = 1:config.syncPackets
                ...
                % Continuously transmit SYNC waveform for syncInterval
                % so the RX has enough time to detect and decode it
                transmitRepeat(tx, txWaveforms{k}, config.syncInterval);
            ...
            for k = (config.syncPackets + 1):totalPkts
                ...
                if isTailPacket
                    for tailRep = 1:config.tailRepeats
                        % Continuously transmit tail packet for finalDelay
                        transmitRepeat(tx, txWaveforms{k}, config.finalDelay);
                    end
                    logMsg(sprintf('[TX] TAIL R%d: pkt %d/%d (x%d) PAPR=%.2fdB', ...
                        repeatIdx, dataIdx, numDataPackets, config.tailRepeats, currentPAPR));
                else
                    % Continuously transmit data packet for txTime
                    transmitRepeat(tx, txWaveforms{k}, config.txTime);
                end
...
txPAPRLabel.Text = sprintf('%.2f dB', currentPAPR);
        ...
        if config.eosMarkers > 0 && ~getappdata(fig, 'stopTx')
            ...
            for i = 1:config.eosMarkers
                % Continuously transmit EOS waveform for 3x finalDelay
                transmitRepeat(tx, eosWaveforms{i}, 3 * config.finalDelay);
...
function runReception(fig, config)
    ...
    % ============================================================
    % PHASE 1: Bulk capture all raw IQ frames into memory
    % No decoding during capture to avoid overruns
    % ============================================================
    rxStatus.Text = 'Capturing...';
    
    maxCaptureFrames = 10000;  % Safety limit
    capturedFrames = cell(maxCaptureFrames, 1);
    capturedFrameIdx = zeros(maxCaptureFrames, 1);  % Temporal frame index for adjacency tracking
    capturedCount = 0;  % Count of actually stored frames
    totalFrames = 0;
    overrunTotal = 0;
    overrunLogLimit = 5;
    ...
    while toc(tStart) < config.rxTimeout && ~getappdata(fig, 'stopRx')
        ...
        [rxSig, hasValidFrame, overrunCount] = receiveRadioFrame(rx);
        
        if overrunCount > 0
            overrunTotal = overrunTotal + 1;
            if overrunTotal <= overrunLogLimit
                logMsg(sprintf('[RX] Warning: overrun detected on frame %d', totalFrames));
                if overrunTotal == overrunLogLimit
                    logMsg('[RX] Further overrun warnings suppressed (will summarize at end)');
                end
            end
        end
        
        if hasValidFrame && ~isempty(rxSig)
            % Quick energy check: only store frames with detectable signal
            % This drastically reduces memory usage
            peakVal = max(abs(rxSig));
            if peakVal > 0.005 && capturedCount < maxCaptureFrames
                capturedCount = capturedCount + 1;
                capturedFrames{capturedCount} = rxSig;
                capturedFrameIdx(capturedCount) = totalFrames;
            end
        end
        
        % Status update (minimal overhead - no drawnow)
        if toc(lastStatus) > 2.0
            logMsg(sprintf('[RX] Capturing... %.1fs (%d frames, %d stored, %d overruns)', ...
                toc(tStart), totalFrames, capturedCount, overrunTotal));
            lastStatus = tic;
            drawnow;
        end
    end
    ...
    % Log overrun summary
    if overrunTotal > 0
        logMsg(sprintf('[RX] Total overruns: %d out of %d frames (%.1f%%)', ...
            overrunTotal, totalFrames, 100*overrunTotal/totalFrames));
    end
    
    % Trim captured frames
    capturedFrames = capturedFrames(1:capturedCount);
    capturedFrameIdx = capturedFrameIdx(1:capturedCount);
    logMsg(sprintf('[RX] Captured %d frames with signal out of %d total. Starting decode...', capturedCount, totalFrames));
    drawnow;
    
    % ============================================================
    % PHASE 2: Decode all captured frames offline
    % No USRP interaction, no overrun risk
    % ============================================================
    rxStatus.Text = 'Decoding...';
    logMsg(sprintf('[RX] Phase 2: Decoding %d captured frames...', length(capturedFrames)));
    drawnow;
    ...
    preConfirmBuffer = {};  % Buffer data payloads received before SYNC lock
    ...
    for frameIdx = 1:length(capturedFrames)
        if getappdata(fig, 'stopRx')
            break;
        end
        
        rxSig = capturedFrames{frameIdx};
        
        % Concatenate with next temporally-adjacent frame for
        % cross-boundary packet detection.  A WLAN packet (~60k
        % samples at CBW5) may straddle two 80k-sample frames.
        if frameIdx < length(capturedFrames) && ...
                capturedFrameIdx(frameIdx+1) == capturedFrameIdx(frameIdx) + 1
            rxSig = [rxSig; capturedFrames{frameIdx+1}]; %#ok<AGROW>
        end
        
        % Free the captured frame to reduce memory pressure
        capturedFrames{frameIdx} = [];
        
        try
            [decoded, payload, errorsBefore, errorsAfter] = tryDecodeWithErrors(rxSig, cfgPHY, config.sampleRate, config);
        catch decodeErr
            if frameIdx <= 3
                logMsg(sprintf('[RX] Decode error on frame %d: %s', frameIdx, decodeErr.message));
            end
            continue;
        end
        ...
        if decoded
            ...
            if length(payload) >= 4 && strcmp(char(payload(1:4)'), 'SYNC')
                ...
                if validSync && ~streamConfirmed
                    ...
                    if confirmCount == 1
                        ...
                        % Lock stream immediately after first valid SYNC
                        ...
                        % Backfill any data packets received before SYNC
                        if ~isempty(preConfirmBuffer)
                            backfilled = 0;
                            for bi = 1:length(preConfirmBuffer)
                                bufPayload = preConfirmBuffer{bi};
                                if length(bufPayload) >= 4
                                    bufSeq = double(typecast(uint8(bufPayload(1:2)), 'uint16'));
                                    if bufSeq > 0 && bufSeq <= totalPkts && ~isKey(rawPayloads, uint32(bufSeq))
                                        rawPayloads(uint32(bufSeq)) = bufPayload;
                                        backfilled = backfilled + 1;
                                    end
                                end
                            end
                            if backfilled > 0
                                logMsg(sprintf('[RX] Backfilled %d pre-SYNC packets', backfilled));
                            end
                            preConfirmBuffer = {};
                        end
                    end
            ...
            if length(payload) >= 4 && strcmp(char(payload(1:4)'), 'EOS!')
                ...
                % In batch mode, remaining frames are already captured —
                % just continue decoding the rest of the buffer
                continue;
            end
            
            % Handle DATA packet
            if length(payload) >= 4
                % Buffer data before stream is confirmed
                if ~streamConfirmed
                    preConfirmBuffer{end+1} = payload; %#ok<AGROW>
                    continue;
                end
        ...
        % Decode progress update
        if mod(frameIdx, 100) == 0 || frameIdx == length(capturedFrames) || frameIdx <= 5
            if streamConfirmed
                logMsg(sprintf('[RX] Decode: frame %d/%d | %d/%d pkts (%.0f%%) | %d decoded', ...
                    frameIdx, length(capturedFrames), rawPayloads.Count, totalPkts, ...
                    100*rawPayloads.Count/totalPkts, totalDecodes));
            else
                logMsg(sprintf('[RX] Decode: frame %d/%d (searching for SYNC... %d decoded so far)', ...
                    frameIdx, length(capturedFrames), totalDecodes));
            end
            drawnow;
        end
    end
...
function tx = createTxRadio(config)
    validateUsrpSupport();
    usrpConfig = buildUsrpRadioConfig(config.txRadioID, config.centerFreq, config.sampleRate);

    txArgs = {'Platform', usrpConfig.platform, ...
        'CenterFrequency', config.centerFreq, ...
        'MasterClockRate', usrpConfig.masterClockRate, ...
        'InterpolationFactor', usrpConfig.interpDecim, ...
        'Gain', 75, ...
        'TransportDataType', 'int16'};

    if ~isempty(usrpConfig.serialNum)
        txArgs = [txArgs, {'SerialNum', usrpConfig.serialNum}];
    elseif ~isempty(usrpConfig.ipAddress)
        txArgs = [txArgs, {'IPAddress', usrpConfig.ipAddress}];
    end

    tx = comm.SDRuTransmitter(txArgs{:});
end

function rx = createRxRadio(config)
    validateUsrpSupport();
    usrpConfig = buildUsrpRadioConfig(config.rxRadioID, config.centerFreq, config.sampleRate);

    rxArgs = {'Platform', usrpConfig.platform, ...
        'CenterFrequency', config.centerFreq, ...
        'MasterClockRate', usrpConfig.masterClockRate, ...
        'DecimationFactor', usrpConfig.interpDecim, ...
        'Gain', config.rxGain, ...
    ...
    if ~isempty(usrpConfig.serialNum)
        rxArgs = [rxArgs, {'SerialNum', usrpConfig.serialNum}];
    elseif ~isempty(usrpConfig.ipAddress)
        rxArgs = [rxArgs, {'IPAddress', usrpConfig.ipAddress}];
    end

    rx = comm.SDRuReceiver(rxArgs{:});
    rx.ReceiveAntennaPort = 'RX2';
end

function usrpConfig = buildUsrpRadioConfig(radioID, centerFreq, sampleRate)
    if isstring(radioID)
        radioID = char(radioID);
    end

    usrpConfig = struct();
    usrpConfig.platform = 'B200';
    usrpConfig.serialNum = '';
    usrpConfig.ipAddress = '';

    if startsWith(radioID, 'serial=')
        usrpConfig.serialNum = char(extractAfter(radioID, 'serial='));
    elseif startsWith(radioID, 'addr=')
        usrpConfig.ipAddress = char(extractAfter(radioID, 'addr='));
    elseif ~strcmp(radioID, 'type=b200')
        usrpConfig.serialNum = radioID;
    end

    usrpConfig.masterClockRate = selectUsrpMasterClockRate(sampleRate);
    usrpConfig.interpDecim = round(usrpConfig.masterClockRate / sampleRate);

    if centerFreq < 70e6
        error('USRP B200 center frequency must be at least 70 MHz');
    end
end

function validateUsrpSupport()
    if exist('findsdru', 'file') == 0 && exist('findsdru', 'builtin') == 0
        error(['USRP MATLAB support is not installed. Install the Communications Toolbox ' ...
            'Support Package for USRP Radio to use a B200 from MATLAB.']);
    end

    if exist('comm.SDRuTransmitter', 'class') == 0 || exist('comm.SDRuReceiver', 'class') == 0
        error(['USRP MATLAB support is not available in this installation. The current SDR interface ' ...
            'supports Pluto/E3xx, but B200 requires comm.SDRuTransmitter and comm.SDRuReceiver.']);
    end
end

function sampleRate = alignSampleRateWithChannelBW(channelBW, requestedSampleRate)
    switch char(channelBW)
        case 'CBW20'
            sampleRate = 20e6;
        case 'CBW40'
            sampleRate = 40e6;
...
function masterClockRate = selectUsrpMasterClockRate(sampleRate)
    minMasterClockRate = 5e6;
    maxMasterClockRate = 61.44e6;

    if sampleRate <= 0
        error('USRP B200 sample rate must be positive');
    end

    interpDecim = max(1, ceil(minMasterClockRate / sampleRate));
    masterClockRate = sampleRate * interpDecim;

    % B200 master clock must be between 5 MHz and 61.44 MHz
    if masterClockRate < minMasterClockRate
        masterClockRate = minMasterClockRate;
    end
    if masterClockRate > maxMasterClockRate
        error('USRP B200 sample rate %.2f MS/s is outside the supported master clock range', sampleRate / 1e6);
    end
end

function transmitRepeat(tx, waveform, duration)
    % Continuously transmit the waveform for the specified duration.
    % Unlike Pluto's built-in transmitRepeat which loops in the FPGA,
    % USRP's comm.SDRuTransmitter sends one buffer per call.
    % We loop to provide continuous RF output so the RX can capture
    % the packet across multiple frame boundaries.
...
function [rxSig, hasValidFrame, overrunCount] = receiveRadioFrame(rx)
    [rxSig, dataLen, overrunCount] = rx();
    hasValidFrame = dataLen > 0;

    % Do NOT truncate rxSig based on dataLen — wlanPacketDetect needs
    % the full buffer to search for preambles. Truncating can clip
    % packets that start partway through the frame.
...
function [ok, payload, errorsBefore, errorsAfter] = tryDecodeWithErrors(rxSig, cfgPHY, fsRF, config)
    ...
    % Cache PhaseFrequencyOffset object to avoid expensive re-creation
    persistent pfOffsetCache pfOffsetFs;
    if isempty(pfOffsetCache) || isempty(pfOffsetFs) || pfOffsetFs ~= fsRF
        pfOffsetCache = comm.PhaseFrequencyOffset('SampleRate', fsRF, 'FrequencyOffsetSource', 'Input port');
        pfOffsetFs = fsRF;
    end
    ...
    try
        ...
        release(pfOffsetCache);
        rxCFO = pfOffsetCache(rxSig, -coarseCFO);
        ...
        release(pfOffsetCache);
        rxSync = pfOffsetCache(rxCFO, -fineCFO);
        ...
        [recLSIG, failCheck] = wlanLSIGRecover(lsig, H, noiseVar, cfgPHY.ChannelBandwidth);
        if failCheck; return; end
        
        % Use fixed PSDULength matching what the TX sends
        % The TX pads all packets to the same max size then wraps with
        % wlanMACFrame (adds 24-byte header + 4-byte FCS), so PSDULength
        % is consistently maxPayloadBytes + 24 + 4 = 2200 + 24 + 4 = 2228
        % Use 2304 + 24 + 4 = 2332 as a safe upper bound
        cfgPHY2 = cfgPHY;
